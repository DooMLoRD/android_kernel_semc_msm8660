...
static int msm_snddev_enable_amic_power(void)
{
	...
	ret = pmic8058_read(PM8058_OTHC_CNTR_BASE0 + 1, &reg, 1);
	if (ret < 0) {
		pr_err("%s: pmic8058_read failed\n", __func__);
	}

	reg &= PM8058_OTHC_EN_SIG_MASK;
	reg |= (OTHC_SIGNAL_ALWAYS_ON << PM8058_OTHC_EN_SIG_SHIFT);

	ret = pmic8058_write(PM8058_OTHC_CNTR_BASE0 + 1, &reg, 1);
	if (ret < 0) {
		pr_err("%s: pmic8058_write failed\n", __func__);
	}
#endif
	return ret;
}
...
static struct adie_codec_action_unit dual_mic_spkr_endfire_8KHz_osr256_actions[] =
...
static struct adie_codec_hwsetting_entry dual_mic_spkr_endfire_settings[] = {
	{
		...
		.action_sz = ARRAY_SIZE(dual_mic_spkr_endfire_8KHz_osr256_actions),
	}
};
...
static struct adie_codec_action_unit ispkr_dual_mic_endfire_tx_real_stereo_48KHz_osr256_actions[] =
	SPKR_AMIC_STEREO_OSR_256;

static struct adie_codec_hwsetting_entry ispkr_dual_mic_endfire_tx_real_stereo_settings[] = {
	{
		.freq_plan = 48000,
		.osr = 256,
		.actions = ispkr_dual_mic_endfire_tx_real_stereo_48KHz_osr256_actions,
		.action_sz =
		ARRAY_SIZE(ispkr_dual_mic_endfire_tx_real_stereo_48KHz_osr256_actions),
	}
};

static struct adie_codec_dev_profile ispkr_dual_mic_endfire_tx_real_stereo_profile = {
	.path_type = ADIE_CODEC_TX,
	.settings = ispkr_dual_mic_endfire_tx_real_stereo_settings,
	.setting_sz = ARRAY_SIZE(ispkr_dual_mic_endfire_tx_real_stereo_settings),
};

static struct snddev_icodec_data snddev_ispkr_dual_mic_endfire_tx_real_stereo_data = {
	.capability = (SNDDEV_CAP_TX | SNDDEV_CAP_VOICE),
...
static struct platform_device msm_ispkr_dual_mic_endfire_tx_real_stereo_device = {
	.name = "snddev_icodec",
	.dev = { .platform_data = &snddev_ispkr_dual_mic_endfire_tx_real_stereo_data },
};
...
static struct platform_device msm_iringtone_xloud_speaker_mono_rx_device = {
	.name = "snddev_icodec",
	.dev = { .platform_data = &snddev_iringtone_xloud_speaker_mono_rx_data },
};
