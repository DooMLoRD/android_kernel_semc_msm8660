...
int pn544_driver_loaded(void)
{
	...
	if (!IS_ERR_VALUE(ret))
		ret = pn544_pm8058_config(PM_NFC_VDDLDO_MON_LEVEL |
			PM_NFC_VPH_PWR_EN | PM_NFC_EXT_VDDLDO_EN, false);

	if (!IS_ERR_VALUE(ret))
		ret = pn544_pm8058_config(PM_NFC_SUPPORT_EN | PM_NFC_EN, true);
...
{
	return pn544_pm8058_config(PM_NFC_LDO_EN, true);
}

void pn544_driver_closed(void)
{
	pn544_pm8058_config(PM_NFC_LDO_EN, false);
}
