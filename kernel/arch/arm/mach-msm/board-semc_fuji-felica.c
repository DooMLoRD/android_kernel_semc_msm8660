/* kernel/arch/arm/mach-msm/board-semc_fuji-felica.c
 *
...
static struct pm8058_gpio pm_felica_pon = {
...
static struct pm8058_gpio pm_felica_rfs = {
...
static struct pm8058_gpio pm_felica_int = {
	.direction      = PM_GPIO_DIR_IN,
	.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
	.pull           = PM_GPIO_PULL_NO,
	.vin_sel        = PM_GPIO_VIN_S3,
...
static int semc_felica_cen_init(struct felica_dev *dev)
{
	...
	ret = pm8058_nfc_get_status(nfcdev, PM_NFC_CTRL_REQ, &st);
	if (ret) {
		dev_err(dev->dev,
...
static int semc_felica_cen_write(u8 arg, struct felica_dev *dev)
{
	...
	ret = pm8058_nfc_config(nfcdev, PM_NFC_EN, val);
	if (ret) {
		dev_err(dev->device_cen.this_device,
