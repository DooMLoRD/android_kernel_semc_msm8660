...
#define PM8058_OTHC_HIGH_CURR			0xF0
#define PM8058_OTHC_EN_SIG				0xC0
#define PM8058_OTHC_HYST_PREDIV			0x38
#define PM8058_OTHC_CLK_PREDIV			0x07
#define PM8058_OTHC_HYST_CLK			0xF0
#define PM8058_OTHC_PERIOD_CLK			0x0F
...
#define OTHC_CONTREG1(addr)			(addr + 1)
#define OTHC_CONTREG2(addr)			(addr + 2)
...
static int simple_remote_pf_enable_mic_bias(unsigned int enable)
{
	int rc = 0;
	u8 reg;
	u16 base_addr = loc_dat->jack_pf->othc_base;
	struct pm8058_chip *pm_chip = loc_dat->pm_chip;
	...
	if (enable) {
		if (!loc_dat->mic_bias_enable_counter) {
			rc = pm8058_read(pm_chip,
				OTHC_CONTREG1(base_addr), &reg, 1);
			if (rc < 0)
				dev_err(loc_dat->dev, "PM8058 read failed\n");

			reg &= PM8058_OTHC_EN_SIG_MASK;
			reg |= (OTHC_SIGNAL_PWM_TCXO <<
				 PM8058_OTHC_EN_SIG_SHIFT);

			rc = pm8058_write(pm_chip,
				OTHC_CONTREG1(base_addr), &reg, 1);
			if (rc < 0)
...
static int simple_remote_pf_set_period_freq(unsigned int value)
{
	...
	ret = pm8058_read(loc_dat->pm_chip,
			OTHC_CONTREG1(base_addr), &reg, 1);
	if (ret < 0) {
		dev_err(loc_dat->dev, "PM8058 read failed\n");
		return ret;
	...
	reg = (reg &  PM8058_OTHC_CLK_PREDIV_MASK) | (ret_val);

	ret = pm8058_write(loc_dat->pm_chip,
			OTHC_CONTREG1(base_addr), &reg, 1);
	if (ret < 0) {
		dev_err(loc_dat->dev, "PM8058 write failed\n");
		return ret;
...
static int simple_remote_pf_set_period_time(unsigned int value)
{
	...
	ret = pm8058_read(loc_dat->pm_chip,
			OTHC_CONTREG2(base_addr) , &reg, 1);
	if (ret < 0) {
		dev_err(loc_dat->dev, "PM8058 read failed\n");
		return ret;
	}

	reg = (reg & PM8058_OTHC_PERIOD_CLK_MASK) | ret_val;
...
static int simple_remote_pf_set_hysteresis_freq(unsigned int value)
{
	...
	reg |= (ret_val << PM8058_OTHC_HYST_PREDIV_SHIFT);
...
static int simple_remote_pf_set_hysteresis_time(unsigned int value)
{
	...
	reg |= ret_val << PM8058_OTHC_HYST_CLK_SHIFT;
...
static int simple_remote_pf_set_trig_level(unsigned int value)
{
	...
	if (value >= 200 && value <= 1700) {
		...
		data = CALC_LEVEL_TO_REG(value);
		reg =  (reg & PM8058_OTHC_HIGH_CURR_MASK) | data;
...
static int simple_remote_pf_get_period_freq(unsigned int *value)
{
	...
	reg = (reg &  PM8058_OTHC_CLK_PREDIV);
...
static int simple_remote_pf_get_period_time(unsigned int *value)
{
	...
	reg = (reg & PM8058_OTHC_PERIOD_CLK_MASK);
...
static int simple_remote_pf_get_hysteresis_freq(unsigned int *value)
{
	...
	reg &= PM8058_OTHC_HYST_PREDIV;
	reg = (reg >> PM8058_OTHC_HYST_PREDIV_SHIFT);
...
static int simple_remote_pf_get_hysteresis_time(unsigned int *value)
{
	...
	reg &= PM8058_OTHC_HYST_CLK;
	reg = reg >> PM8058_OTHC_HYST_CLK_SHIFT;
...
static int simple_remote_pf_probe(struct platform_device *pdev)
{
	...
	loc_dat->pm_chip = platform_get_drvdata(pdev);
	if (loc_dat->pm_chip == NULL) {
		ret = -EINVAL;
		goto exit_device_put;
	}
