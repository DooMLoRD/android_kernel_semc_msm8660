/* drivers/video/msm/hdmi_msm_fuji.c
 *
...
void hdmi_msm_cec_msg_send(struct hdmi_msm_cec_msg *msg)
{
	...
	if (!timeout_count) {
		...
		DEV_ERR("hdmi_msm_bvs_msg_send: timedout");
...
void hdmi_msm_cec_line_latch_detect(void)
{
	...
	if (hdmi_msm_state->first_monitor == 1) {
		DEV_DBG("CEC line is probably latched up - CECT 9-5-1");
		HDMI_OUTP(0x028C, 0x0);
		HDMI_OUTP(0x028C, HDMI_MSM_CEC_CTRL_ENABLE);
		hdmi_msm_state->first_monitor = 0;
	}
...
void hdmi_msm_cec_msg_recv(void)
{
	...
	if (hdmi_msm_state->cec_queue_wr == hdmi_msm_state->cec_queue_rd
		...
		mutex_unlock(&hdmi_msm_state_mutex);
		DEV_ERR("BVS message queue is overflowing\n");
#ifdef DRVR_ONLY_CECT_NO_DAEMON
		/*
		 * Without CEC daemon:
	...
	if (hdmi_msm_state->cec_queue_wr == NULL) {
		DEV_ERR("hdmi_msm_bvs_msg_recv: wp is NULL\n");
		return;
	}
	mutex_unlock(&hdmi_msm_state_mutex);
	...
	if (hdmi_msm_state->cec_queue_wr->frame_size < 1) {
		DEV_ERR("hdmi_msm_bvs_msg_recv: invalid message "
			"(frame length = %d)",
			hdmi_msm_state->cec_queue_wr->frame_size);
		return;
...
static void hdmi_msm_hpd_state_work(struct work_struct *work)
{
	...
	if (!hdmi_msm_state->hpd_cable_chg_detected) {
		...
		if (hpd_state) {
			...
			/* notify userspace */
			mutex_lock(&hdmi_msm_state_mutex);
			hdmi_msm_state->int_status |= IRQ_HPD;
			mutex_unlock(&hdmi_msm_state_mutex);
			complete(&hdmi_msm_state->int_done);
		}
	} else {
...
static void hdmi_msm_hdcp_work(struct work_struct *work)
{
	...
	/* Only re-enable if panel power on */
	mutex_lock(&hdmi_msm_state_mutex);
	if (hdmi_msm_state->panel_power_in_transition_off ||
	    !(hdmi_msm_state->panel_power_on) ||
	    !(hdmi_msm_state->hpd_stable)) {
		mutex_unlock(&hdmi_msm_state_mutex);
		DEV_WARN("HDCP: Re-enable skipped, panel power off %d|%d\n",
			hdmi_msm_state->panel_power_in_transition_off,
			!(hdmi_msm_state->panel_power_on));
		return;
	}
	mutex_unlock(&hdmi_msm_state_mutex);
...
static irqreturn_t hdmi_msm_isr(int irq, void *dev_id)
{
	...
	if ((hdcp_int_val & (1 << 6)) && (hdcp_int_val & (1 << 4))) {
		...
		DEV_INFO("HDCP: AUTH_FAIL_INT received, LINK0_STATUS=0x%08x\n",
			HDMI_INP_ND(0x011C));
		if (hdmi_msm_state->full_auth_done) {
			/* Only handle IRQ if panel power on */
			if (!(hdmi_msm_state->panel_power_in_transition_off) &&
			    hdmi_msm_state->panel_power_on) {
				envp[0] = "HDCP_STATE=FAIL";
				envp[1] = NULL;
				...
				DEV_INFO("HDCP: ignored, panel power off\n");
		}
		mutex_lock(&hdcp_auth_state_mutex);
	...
	DEV_DBG("cec interrupt status is [0x%x]\n", cec_intr_status);
	...
	if ((cec_intr_status & (1 << 4)) && (cec_intr_status & (1 << 5))) {
		...
		if (hdmi_msm_state->first_monitor == 0) {
			...
			/* OEM can change this timer value based on testing
			   worst case = 16 bytes * 8 * 2.7msec = 346 msec */
...
static void hdcp_deauthenticate(void)
{
	...
	/* notify userspace */
	/* after hdcp_active to be FALSE */
	mutex_lock(&hdmi_msm_state_mutex);
	hdmi_msm_state->int_status |= IRQ_AUTH_STS_CHG;
	mutex_unlock(&hdmi_msm_state_mutex);
	complete(&hdmi_msm_state->int_done);
	mutex_lock(&hdcp_auth_state_mutex);
	hdmi_msm_state->full_auth_done = FALSE;
...
static void hdmi_msm_hdcp_enable(void)
{
	...
	* We probably need to protect this in a mutex lock */
	...
	/* Wait until TMDS signal is stable */
	msleep(200);
...
error:
	...
	if (hdmi_msm_state->hpd_during_auth) {
		...
		hdcp_deauthenticate();
		/* hpd_during_auth will be FALSE in */
		/* hdmi_msm_hdcp_enable called on next HPD event */
		/* with hpd_during_auth=TRUE */
	} else {
...
static uint8 hdmi_msm_underscan;
...
#define HDMI_SPD_HEADER_SIZE	4
#define HDMI_SPD_DATA_SIZE	25
#define HDMI_SPD_MAX_DATA_SIZE	(HDMI_SPD_HEADER_SIZE + HDMI_SPD_DATA_SIZE)

static uint8 spd_infoframe_data[HDMI_SPD_MAX_DATA_SIZE];
...
static void hdmi_msm_spd_infoframe_packetsetup(void)
{
	...
	*/
	packet_header  = ((spd_infoframe_data[0] & 0xff)
		| ((spd_infoframe_data[1] & 0xff) << 8)
		| ((spd_infoframe_data[2] & 0xff) << 16));
	...
	packet_payload = ((spd_infoframe_data[3] & 0xff)
		| ((spd_infoframe_data[4] & 0xff) << 8)
		| ((spd_infoframe_data[5] & 0xff) << 16)
		| ((spd_infoframe_data[6] & 0xff) << 24));
	...
	packet_payload = ((spd_infoframe_data[7] & 0xff)
		| ((spd_infoframe_data[8] & 0xff) << 8)
		| ((spd_infoframe_data[9] & 0xff) << 16)
		| ((spd_infoframe_data[10] & 0xff) << 24));
	...
	packet_payload = ((spd_infoframe_data[11] & 0xff)
		| ((spd_infoframe_data[12] & 0xff) << 8)
		| ((spd_infoframe_data[13] & 0xff) << 16)
		| ((spd_infoframe_data[14] & 0xff) << 24));
	...
	packet_payload = ((spd_infoframe_data[15] & 0xff)
		| ((spd_infoframe_data[16] & 0xff) << 8)
		| ((spd_infoframe_data[17] & 0xff) << 16)
		| ((spd_infoframe_data[18] & 0xff) << 24));
	...
	packet_payload = ((spd_infoframe_data[19] & 0xff)
		| ((spd_infoframe_data[20] & 0xff) << 8)
		| ((spd_infoframe_data[21] & 0xff) << 16)
		| ((spd_infoframe_data[22] & 0xff) << 24));
	...
	packet_payload = ((spd_infoframe_data[23] & 0xff)
		| ((spd_infoframe_data[24] & 0xff) << 8)
		| ((spd_infoframe_data[25] & 0xff) << 16)
		| ((spd_infoframe_data[26] & 0xff) << 24));
	 ...
	 * 08h Game
	 * 09h PC general */
	packet_payload = ((spd_infoframe_data[27] & 0xff)
		| ((spd_infoframe_data[28] & 0xff) << 8));
	HDMI_OUTP(0x00C0, packet_payload);

	packet_control = HDMI_INP_ND(0x0034);

	/* GENERIC1_LINE | GENERIC1_UPDATE | GENERIC1_CONT | GENERIC1_SEND
	 * Setup HDMI TX generic packet control
	 * Enable this packet to transmit every frame
	 * Enable this packet to transmit every frame
	 * Enable HDMI TX engine to transmit Generic packet 0 */
	packet_control |= ((0x1 << 24) | (1 << 6) | (1 << 5) | (1 << 4));
	HDMI_OUTP(0x0034, packet_control);
}
...
static int hdmi_msm_power_on(struct platform_device *pdev)
{
	...
	hdmi_msm_state->panel_power_on = TRUE;
#ifndef CONFIG_FB_MSM_HDMI_MSM_PANEL_HDCP_SUPPORT
	/* make a flag TRUE to carry out hdmi_msm_turn_on() by all means */
	changed = TRUE;
#endif /* CONFIG_FB_MSM_HDMI_MSM_PANEL_HDCP_SUPPORT */
...
#ifdef CONFIG_FB_MSM_HDMI_MSM_PANEL_HDCP_SUPPORT
	/* enable HDCP after panel power on */
	mod_timer(&hdmi_msm_state->hdcp_timer, jiffies + HZ/2);
#endif /* CONFIG_FB_MSM_HDMI_MSM_PANEL_HDCP_SUPPORT */
...
static int hdmi_msm_power_off(struct platform_device *pdev)
{
	...
	hdmi_msm_state->panel_power_in_transition_off = TRUE;
	...
	hdmi_msm_audio_off();
#ifdef CONFIG_FB_MSM_HDMI_MSM_PANEL_HDCP_SUPPORT
	/* make sure all HDCP tasks to be flushed and */
	/* not to be scheduled after panel power off */
	mod_timer(&hdmi_msm_state->hdcp_timer, 0xffffffffL);
	flush_workqueue(hdmi_work_queue);
	hdcp_deauthenticate();
#endif
	if (timer_pending(&hdmi_msm_state->hpd_state_timer))
		hpdtimer = 1;
	hdmi_msm_hpd_off();
	hdmi_msm_powerdown_phy();
	hdmi_msm_dump_regs("HDMI-OFF: ");
	/* restart hpd timer for short low */
	if (hpdtimer)
	...
	hdmi_msm_state->panel_power_on = FALSE;
	hdmi_msm_state->panel_power_in_transition_off = FALSE;
...
static ssize_t hdmi_msm_irq_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	int timeout = 0;
	mutex_lock(&hdmi_msm_state_mutex);
	if (hdmi_msm_state->int_status == 0) {
		mutex_unlock(&hdmi_msm_state_mutex);
		INIT_COMPLETION(hdmi_msm_state->int_done);
		timeout = wait_for_completion_interruptible
					(&hdmi_msm_state->int_done);
	...
	mutex_lock(&hdmi_msm_state_mutex);
	DEV_DBG("%s : HPD=%d RSEN=%d CEC=%d HDCP=%d\n",
		__func__,
		!!(hdmi_msm_state->int_status & IRQ_HPD),
		!!(hdmi_msm_state->int_status & IRQ_RSEN),
		!!(hdmi_msm_state->int_status & IRQ_BVS),
		!!(hdmi_msm_state->int_status & IRQ_AUTH_STS_CHG));

	memcpy(buf, &hdmi_msm_state->int_status,
		sizeof(hdmi_msm_state->int_status));

	hdmi_msm_state->int_status = 0;
	mutex_unlock(&hdmi_msm_state_mutex);

	return sizeof(hdmi_msm_state->int_status);
}

static ssize_t hdmi_msm_rda_edid_block0(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	__u8 data[0x80];

	msm_hdmi_init_ddc();
	hdmi_msm_read_edid_block(0, data);
	memcpy(buf, &data, 0x80);
	return 0x80;
}

static ssize_t hdmi_msm_rda_edid_block1(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	__u8 data[0x80];

	msm_hdmi_init_ddc();
	hdmi_msm_read_edid_block(1, data);
	memcpy(buf, &data, 0x80);
	return 0x80;
}

static ssize_t hdmi_msm_rda_edid_block2(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	__u8 data[0x80];

	msm_hdmi_init_ddc();
	hdmi_msm_read_edid_block(2, data);
	memcpy(buf, &data, 0x80);
	return 0x80;
}

static ssize_t hdmi_msm_rda_edid_block3(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	__u8 data[0x80];

	msm_hdmi_init_ddc();
	hdmi_msm_read_edid_block(3, data);
	memcpy(buf, &data, 0x80);
	return 0x80;
}

static ssize_t hdmi_msm_rda_edid_block128(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	__u8 data[0x80];

	msm_hdmi_init_ddc();
	hdmi_msm_read_edid_block(128, data);
	memcpy(buf, &data, 0x80);
	return 0x80;
}

static ssize_t hdmi_msm_wta_underscan(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = strnlen(buf, PAGE_SIZE);
	int underscan = atoi(buf);

	if (underscan)
		hdmi_msm_underscan = 0x02;
	else
		hdmi_msm_underscan = 0x00;

	return ret;
}

static ssize_t hdmi_msm_wta_spd_frame(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	memcpy(spd_infoframe_data, buf, HDMI_SPD_MAX_DATA_SIZE);
	mutex_lock(&external_common_state_hpd_mutex);
	if (external_common_state->hpd_state)
		hdmi_msm_spd_infoframe_packetsetup();
	mutex_unlock(&external_common_state_hpd_mutex);

	return HDMI_SPD_MAX_DATA_SIZE;
}

static DEVICE_ATTR(hdmi_msm_irq, S_IRUGO,
		   hdmi_msm_irq_show, NULL);

static DEVICE_ATTR(edid_block0, S_IRUGO,
	hdmi_msm_rda_edid_block0, NULL);

static DEVICE_ATTR(edid_block1, S_IRUGO,
	hdmi_msm_rda_edid_block1, NULL);

static DEVICE_ATTR(edid_block2, S_IRUGO,
	hdmi_msm_rda_edid_block2, NULL);

static DEVICE_ATTR(edid_block3, S_IRUGO,
	hdmi_msm_rda_edid_block3, NULL);

static DEVICE_ATTR(edid_block128, S_IRUGO,
	hdmi_msm_rda_edid_block128, NULL);

static DEVICE_ATTR(wr_underscan, S_IWUGO,
	NULL, hdmi_msm_wta_underscan);

static DEVICE_ATTR(spd_wr_frame, S_IWUGO,
	NULL, hdmi_msm_wta_spd_frame);

static struct attribute *hdmi_msm_attrs[] = {
	&dev_attr_hdmi_msm_irq.attr,
	&dev_attr_edid_block0.attr,
	&dev_attr_edid_block1.attr,
	&dev_attr_edid_block2.attr,
	&dev_attr_edid_block3.attr,
	&dev_attr_edid_block128.attr,
	&dev_attr_wr_underscan.attr,
	&dev_attr_spd_wr_frame.attr,
	NULL,
};

static struct attribute_group hdmi_msm_attr_group = {
	.attrs = hdmi_msm_attrs,
};

static int __devinit hdmi_msm_state_create(struct platform_device *pdev)
{
	int rc;
	struct msm_fb_data_type *mfd = platform_get_drvdata(pdev);
	if (!mfd) {
	...
	rc = sysfs_create_group(&mfd->fbi->dev->kobj, &hdmi_msm_attr_group);
	if (rc) {
	...
	/* external_common */
	return external_common_state_create(pdev);
}

static void hdmi_msm_state_remove(void)
{
...
static int __devinit hdmi_msm_probe(struct platform_device *pdev)
{
	...
	if (!hdmi_msm_state->pd->cec_power) {
		DEV_ERR("Init FAILED: bvs_power function missing\n");
		rc = -ENODEV;
		goto error;
	}
	...
	if (fb_dev) {
		rc = hdmi_msm_state_create(fb_dev);
		if (rc) {
...
static int __init hdmi_msm_init(void)
{
	...
	if (!hdmi_msm_state->cec_queue_start) {
		pr_err("hdmi_msm_init FAILED: BVS queue out of memory\n");
		rc = -ENOMEM;
		goto init_exit;
	}
