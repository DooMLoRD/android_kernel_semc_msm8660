/* drivers/input/misc/akm8975.c - AK8975 compass driver
 *
 * Copyright (C) 2011 Sony Ericsson Mobile Communications AB.
 * Authors: Takashi Shiina <takashi.shiina (at) sonyericsson.com>
 *          Tadashi Kubo <tadashi.kubo (at) sonyericsson.com>
 *          Aleksej Makarov <Aleksej.Makarov (at) sonyericsson.com>
 *          Joachim Holst <joachim.holst (at) sonyericsson.com>
...
#define AKM8975_WAIT_TIME_MS	30
...
static void akm8975_continuous_measure(struct work_struct *work)
{
	...
	if (0 <= interval) {
		...
		/* Following function will check is_busy status */
				   ...
				   const char *buf, size_t count)
{
	...
	if ((astc_value < AK8975_UCHAR_MIN) || (AK8975_UCHAR_MAX < astc_value))
		return -EINVAL;
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* HX: 13-bit */
	input_set_abs_params(akm->input, ABS_RX, -4096, 4095, 0, 0);
	/* HY: 13-bit */
	input_set_abs_params(akm->input, ABS_RY, -4096, 4095, 0, 0);
	/* HZ: 13-bit */
	input_set_abs_params(akm->input, ABS_RZ, -4096, 4095, 0, 0);
	/* ST2 */
	...
	/* Implement continuous measurement by software */
